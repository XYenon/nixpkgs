...
let
  ...
  staticGccRuntime = runCommand "bun-static-gcc-runtime" { } ''
    mkdir -p $out/lib
    libgcc=$(${stdenv.cc}/bin/${stdenv.cc.targetPrefix}cc -print-libgcc-file-name)
    ln -s "$libgcc" $out/lib/libgcc_eh.a
  '';
in
stdenv.mkDerivation {
  pname = "bun-unwrapped";
  ...
  ++ lib.optional isStatic removeReferencesTo
  ...
  env = {
    GIT_SHA = revision;
    NIX_CFLAGS_LINK = lib.optionalString isStatic "-static";
    ...
    BUN_NIX_STATIC = lib.optionalString isStatic "1";
  ...
  buildPhase = ''
    ...
    ${lib.optionalString (!isStatic) ''
      buildArgs+=(--static-libatomic=off)
    ''}
  ...
  postFixup =
    fixDarwinBinary "$out/bin/bun"
    + lib.optionalString isStatic ''
      find $out -type f -exec remove-references-to -t ${bun-webkit} '{}' +
    '';
  ...
  installCheckPhase = ''
    ...
    ${lib.optionalString (isLinux && !isStatic) ''
      CI=1 \
