...
symlinkJoin {
  ...
  nativeBuildInputs = lib.optionals (stdenv.hostPlatform.isLinux && !stdenv.hostPlatform.isStatic) [
    makeBinaryWrapper
  ];

  postBuild = lib.optionalString (stdenv.hostPlatform.isLinux && !stdenv.hostPlatform.isStatic) ''
